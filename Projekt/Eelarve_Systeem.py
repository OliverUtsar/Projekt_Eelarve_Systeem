# 2 kood: Parandatud ja lisatud soovitustega
...
#  Eelistuste akna avamine   
def ava_eelistused_aken():
#   Muutujate loomine   
    eelistus_protsent = ""
    eelistus_kategooria =""
    
#     Eelistuste kinnitamine
    def kinnita_eelisus():
...
#        Kuna kulud_csvst() võib returnida 0 kui cvs-d pole loodud, on siin kontroll
#       kui kategooriad == 0 siis järelikult ei ekisteeri csv-d
        if kategooriad == 0:
...
# väljaminek_summa on kõikide väljaminekute summa kokku            
    try:
...
#  soovitus funktsioon
    def kuva_soovitus():
        # Loeme eelistused CSV-failist
        eelistused_dict = {}
        try:
            with open(failinimetus, mode="r", newline="", encoding="utf-8") as file:
                reader = csv.DictReader(file)
                for row in reader:
                    kategooria = row.get("kategooria")
                    protsent = float(row.get("Protsent kogu kulust", 0))
                    eelistused_dict[kategooria] = protsent
        except FileNotFoundError:
            messagebox.showerror("Viga", "Eelistuste fail puudub. Palun seadistage eelistused!")
            return

        # Loeme tegelikud kulutused
        tegelikud_kulud = kulud_csvst()

        # Kontrollime iga kategooriat
        soovitused = []
        for kategooria, eelistus_protsent in eelistused_dict.items():
            tegelik_summa = tegelikud_kulud.get(kategooria, 0)
            tegelik_protsent = (tegelik_summa / sissetulek) * 100 if sissetulek > 0 else 0

            if tegelik_protsent > eelistus_protsent:
                soovitused.append(f"{kategooria}: Kulutate {tegelik_protsent:.1f}% (soovitatud: {eelistus_protsent:.1f}%). Proovige vähendada oma kulutusi, kas vähem kulutada või otsima paremaid pakkumisi.")
            else:
                soovitused.append(f"{kategooria}: Kulutate {tegelik_protsent:.1f}% (soovitatud: {eelistus_protsent:.1f}%). Hea töö, olete õigel järjel!")

        # Kuvame soovitused
        if soovitused:
            messagebox.showinfo("Soovitused", "\n".join(soovitused))
        else:
            messagebox.showinfo("Soovitused", "Kulutuste ja eelistuste võrdlemiseks puuduvad andmed.")
